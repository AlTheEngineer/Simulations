...
def generate_bias_per_dist_plot(fileNames, roundNum, seqLength, distance):
    ...
    if(distance=="hamming"):
        ...
        cm = plt.cm.gist_ncar
        colors = [cm(i) for i in np.linspace(0.2, 0.9, seqLength-4)]
        markers = ['x', 'p', 's', '^', 'o']
        for i in xrange(seqLength-11):
            y_smooth = spline(roundNumAxis, plotsList[0][i], roundNumAxis_smooth)
            ax.plot(roundNumAxis_smooth, y_smooth, color=colors[i], label=str(i), marker=markers[i%5], markersize=5, markevery=5)
            ax.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
            ax.legend(loc=2, ncol=4, prop={'size':9})
        plt.subplots_adjust(left=0.09, right=0.97, top=0.93, bottom=0.10)        
        fig0.text(0.5, 0.02, 'Round Number', ha='center', size=12)
        fig0.text(0.5, 0.95, 'Total Sequences', ha='center', size=12)
        fig0.text(0.02, 0.5, 'Average Bias', va='center', rotation='vertical', size=12)
        fig0.savefig(str(fileNames)+"_SELEX_Analytics_biasDist_poster", format='pdf', transparent=True)
        return fig0
    else:
